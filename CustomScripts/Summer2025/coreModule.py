...
top_right_vise = [0.27707872509090453,-0.33669320085795396, z_origin_offset_from_sensor] #obtained x and y position manually using precision tip and moving robot around
...
PRINT_PRESSURE=45 #actual printing pressure

STOCK_HEIGHT_OFFSET = 0 #variable to determine offset of stock height from default stock height (default stock height = 0.5 inches)
# ---------------------------------------------------------------------------------------------------------------------------------------------------
...
nearNozzle = [-0.3267, -0.2950, 0.2227, 0, -3.1415, 0]
atNozzle = [-0.4944, -0.2950, 0.2227, 0, 3.1415, 0]
liftNozzle = [-0.4944, -0.2950, 0.3664, 0, 3.1415, 0]

# Waypoints related to grabbing and dropping the ink printer
nearPrinter = [-0.3267, -0.3967, 0.2227, 0, -3.1415, 0]
atPrinter = [-0.4939, -0.3967, 0.2227, 0, -3.1415, 0]
liftPrinter = [-0.4939, -0.3967, 0.3664, 0, -3.1415, 0]
...
def prime_ink(delay=1.0):
    """
    Primes the air line to the ink printer and pressurizes it
    at the specified pressure. Default is 85psi!
    """
    set_pressure(pressure=DEFAULT_PRINT_PRESSURE)
    time.sleep(delay)

# ---------------------------------------------------------------------------------------------------------------------------------------------------
# === # Function to get stock height offset to include varying stock dimensions # === #
def get_stock_height_offset(filepath):
    global STOCK_HEIGHT_OFFSET
    with open(filepath, 'r') as f:
        data = json.load(f)

    for component in data['componentdata']: #go through all component information
        if component['modelName'] == "stock":
            stock_height_inches = component['dimZ']
            stock_height_offset_inches = stock_height_inches - 0.5 #0.5 inches = default stock height
            STOCK_HEIGHT_OFFSET = stock_height_offset_inches / 39.37
            break