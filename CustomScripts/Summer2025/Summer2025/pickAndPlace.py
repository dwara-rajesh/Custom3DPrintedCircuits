...
# filename = r"C:\git\ADML\Automated Circuit Printing and Assembly\Summer2025\DEMO2.json"
...
x_offset_in_mm = 1.0
y_offset_in_mm = 1.0
z_offset_in_m = z_offset_in_mm/1000
z_origin_vac = coreModule.CALIBRATION_DATA['vac'][2] + coreModule.top_right_vise[2] + coreModule.STOCK_HEIGHT_OFFSET - z_offset_in_m #offset the origin by calibration data to avoid hitting the vise and stock height offset to avoid hitting stock
...
COMPONENTS = {'battery': {'start': [91.77, 50.05],   # XY coords of the start of the grid (Bottom left corner)
                            'end': [142.23, 100.62], # XY coords of the end of the grid (Top right corner)
                   ...
                   'pocket_depth': 0.082,            # Max allowed depth into pocket in tray (in inches)
                          ...
                          'engraving_depth': 1.5},  # Depth of cut in stock
                  'led': {'start': [22.40, 66.53],
                            'end': [67.29, 105.97],
               ...
               'button': {'start': [29.55, 13.9],
                   ...
                   'pocket_depth': 0.2,
                          'z_val': 1.0,
                          'engraving_depth': 2.0},
      'microcontroller': {'start': [101.74, 19.88],
                            'end': [139.83, 19.88],
...
def determine_schematic(recentsavefilepath):
    ...
    for component in data['componentdata']: #go through all component information
        if component['modelName'] != "stock": #if its not stock
            ...
            #get position (x,y,z)
            position_x = origin_vac[0] + (component['posX'] * 25.4) - x_offset_in_mm#convert in to mm 
            position_y = origin_vac[1] + (component['posY'] * 25.4) - y_offset_in_mm
            position_z = origin_vac[2] + coreModule.COMPONENT_HEIGHTS[name] - COMPONENTS[name]['engraving_depth']
...
def PNP_Test(): #to test yield for new vacuum heads, ENSURE ALL SLOTS ARE FILLED
    ...
    coreModule.grab_nozzle() #grab nozzle
    coreModule.vacuum_on() #turn on vacuum
    comp_grid = {"button":1,"battery":1,"microcontroller":1,"led":1} #number of components present in each grid
    for key, value in comp_grid.items():
        for i in range(value):
            if i in placed_components[key]:
                pass
            else:
                failed = pickup_component(key,i) #pick component from slot
                if not failed:
...
# # Nozzle Tip Rubber Caps
# # Button: 8/8, Battery: 9/9, Microcontroller: 2/2, LED: 18/20 
# # HENCE Nozzle Tip Rubber Caps BETTER