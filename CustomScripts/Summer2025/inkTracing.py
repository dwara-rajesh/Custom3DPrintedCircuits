...
z_offset = -0.7 #in mm, change value to move nozzle closer or further away from part (increasing value moves it away from part (upward)) 
# (IF STRINGING/COILING DURING EXTRUSION, REDUCE Z_OFFSET VALUE)
# (IF SMEARING DURING EXTRUSION, INCREASE Z_OFFSET VALUE)
...
def meander_terminal(centre, component, k=3,speed=coreModule.precise, reinforce=False):
    ...
    if reinforce:
        k = 7 #increase the layers from 3 to 7
