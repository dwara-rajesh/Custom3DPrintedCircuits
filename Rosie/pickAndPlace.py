...
z_origin_vac = CALIBRATION_DATA['vac'][2] + top_right_vise[2] #offset the origin by calibration data to avoid hitting the vise
origin_in_m_vac = [top_right_vise[0],top_right_vise[1]-admlviceblock_yoff,z_origin_vac, 0, math.pi, 0] #top right corner of stock on vice in rosie station in meters
...
def determine_schematic(recentsavefilepath):
    ...
    for component in data['componentdata']: #go through all component information
        if component['modelName'] != "stock": #if its not stock
            ...
            position_z = origin_vac[2] + COMPONENT_HEIGHTS[name]
...
def move_to_component(component, index, Z_target=2.0, speed=precise):
    ...
    target_pos = [step_x*column_num + pos_data['start'][0],
                  ...
                  Z_target]
    goto_pos(target_pos, speed=speed) #go to position
    return target_pos
...
def pickup_component(component, index, skip_hover=False, print_warnings=False,print_log=False):
    ...
    while get_pressure() > PRESSURE_THRESHOLD and depth_travelled_into_grid < (COMPONENTS[component]['pocket_depth'] * 25.4): # True when object not yet picked up
        ...
        if nozzle_forces[2] <= MAX_FORCE: # Check to ensure that if the vacuum fails, we still stop the nozzle safely!
            ...
            goto_pos(target_pos, speed=micrometric) #move to new position
        else:
    ...
    if get_pressure() > PRESSURE_THRESHOLD: #if pressure is greater than threshold, failed pickup
        with open(r"C:\git\ADML\Automated Circuit Printing and Assembly\Summer2025\rosie_pnp_log.txt", "a") as file:
        ...
        if index >= maxindex: #check if next slot number is great than max slots
            ...
            vacuum_off() #else switch off vacuum
            if print_log:
                with open(r"C:\git\ADML\Automated Circuit Printing and Assembly\Summer2025\rosie_pnp_log.txt", "a") as file:
...
def circuit_pick_and_place(schematic, cycle_vice=False, print_log=False):
    ...
    rtde_control.moveL(standby, speed=fast) #move to standby position
    goto_pos([0,0,100]) #go to origin point of grid (the black crosshair on the bottom left of the components tray)
...
def PNP_Test(): #to test yield for new vacuum heads, ENSURE ALL SLOTS ARE FILLED
    ...
    for key, value in comp_grid.items():
        for i in range(value):
            ...
            pos = rtde_receive.getActualTCPPose() #get current position
